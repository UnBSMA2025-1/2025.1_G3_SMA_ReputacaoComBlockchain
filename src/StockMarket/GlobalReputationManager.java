...
public class GlobalReputationManager {
    ...
    private static final Map<AID, AgentReputationData> reputationDatabase = new HashMap<>();

    public static synchronized void recordInteractionOutcome(AID agentAID, boolean wasSuccessful) {
    	
        reputationDatabase.putIfAbsent(agentAID, new AgentReputationData(agentAID.getLocalName()));
        
        // Get the reputation data for the agent and record the outcome on the db
        AgentReputationData rData = reputationDatabase.get(agentAID);
    ...
    public static synchronized void printAllReputations() {
        ...
        for (Map.Entry<AID, AgentReputationData> entry : reputationDatabase.entrySet()) {
